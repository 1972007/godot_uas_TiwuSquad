...
func _gui_input(event: InputEvent) -> void:
	if has_popup and (event is InputEventMouseButton or event is InputEventScreenTouch):
		if event.button_index == BUTTON_RIGHT and event.pressed:
			if _item_db and _item_db.description:
				_create_properties_popup()
				_popup_ui.show()
	if (event is InputEventMouseButton or event is InputEventScreenTouch) and event.pressed and event.doubleclick:
